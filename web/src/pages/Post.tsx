...
const PostBody = (props: { uri: string }) => {
    ...
    return (
        <>
            ...
            >
                {loading && <PostStatus label="読み込み中..." compact={true} />}
                {!loading && tab === 'replies' && replies.length === 0 && <PostStatus label="リプライはまだありません。" compact={true} />}
                {!loading &&
                    tab === 'replies' &&
                    replies.map((reply) => (
                ...
                {!loading && tab === 'reroutes' && reroutes.length === 0 && <PostStatus label="リルートはまだありません。" compact={true} />}
                {!loading &&
                    tab === 'reroutes' &&
                ...
                {!loading && tab === 'favorites' && favorites.length === 0 && <PostStatus label="お気に入りはまだありません。" compact={true} />}
                {!loading &&
                    tab === 'favorites' &&
                ...
                {!loading && tab === 'reactions' && Object.keys(reactionCounts).length === 0 && <PostStatus label="リアクションはまだありません。" compact={true} />}
                {!loading && tab === 'reactions' && Object.keys(reactionCounts).length > 0 && (
                    <div
                        style={{
                            display: 'flex',
                            flexWrap: 'wrap',
                            ...
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    void client.api
                                        ...
                                        .then((records) => {
                                            setReactionMembers(records.map((record) => Association.fromSignedDocument(record)) as Association<ReactionAssociationSchema>[])
                                        })
                                }}
                                style={{
                                    display: 'flex',
