...
- **`plugins/`** — custom Tauri plugins: `tauri-plugin-keychain` (OS keychain key storage), `tauri-plugin-file-saver`, `tauri-plugin-scroll-killer` (disables WebView-level scrolling on iOS/Android), `tauri-plugin-translation` (on-device translation + language detection: Apple Translation on iOS 18+, ML Kit on Android).
